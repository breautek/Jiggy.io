...
let assetFactory: AssetFactory = AssetFactory.getSingleton();
...
export abstract class AudioEngine extends EventEmitter {
    ...
    private $audioMap: Record<string, Array<Asset>>;
    ...
    public hasAudio(name: string): boolean {
        let audio: Asset = this._getAudio(name);
        return (audio !== null);
    }
    ...
    public releaseAssets(): void {
        let keys = Object.keys(this.$audioMap);
    ...
    protected _setAudio(name: string, audio: Asset, channels: number): void {
        if (!audio) {
            ...
            let channelArr: Array<Asset> = [ audio ];
            if (channels > 1) {
                let clone: Asset;
                for (let i: number = 1; i < channels; i++) {
    ...
    protected _getAudio(name: string, justGiveChannel1?: boolean): Asset {
        if (this.$audioMap[name]) {
            let channels: Array<Asset> = this.$audioMap[name];
            if (justGiveChannel1) {
                return channels[0];
            }
            else {
     ...
     * @return {Object}       
     */
    protected _getData(audio: Asset): any /* TODO, Fix return type */ {
     ...
     * @return {void}              
     */
    protected _registerEvents(channelArray: Array<Asset>, name: string, audio: Asset): void {
        let channel: Asset;
